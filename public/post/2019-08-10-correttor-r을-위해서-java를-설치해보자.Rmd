...
1. `multilinguer::has_java()`를 쓰세요.
1. MacOS는 컴퓨터 비밀번호가 필요합니다.
1. `multilinguer::has_java(force = T)` 해야할 수도 있어요.
...
`has_java()` 함수의 목표는 [rJava][rJava]를 설치하여 문제 없이 동작하게 만드는 것입니다.
`has_java()` 함수는 현재 컴퓨터에 `java`가 설치되어 있는지 확인합니다.
없다면 [corretto][corretto] 설치를 시작합니다.
...
[소스]: https://github.com/mrchypark/correttor/blob/master/R/uncompress.R
